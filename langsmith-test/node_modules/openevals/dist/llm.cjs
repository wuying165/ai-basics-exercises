        ...
        if (_isRunnableInterface(prompt)) {
            ...
            if (attachments !== undefined && splitIdx !== -1) {
                ...
                const renderHalf = async (tmpl) => {
                    if (!tmpl)
                        return "";
                    const t = prompts_1.ChatPromptTemplate.fromTemplate(tmpl);
                ...
                const items = Array.isArray(attachments)
                    ? attachments
                    : [attachments];
                const attachmentBlocks = items.map(utils_js_1._attachmentToContentBlock);
                const content = [
                ...
                if (attachments !== undefined) {
                    ...
                    lastMsg.content = [
                        ...
                        ...items.map(utils_js_1._attachmentToContentBlock),
                    ];
                }
            }
        }
