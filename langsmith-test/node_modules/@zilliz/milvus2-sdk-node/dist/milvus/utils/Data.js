...
const processVectorData = (item, transformers) => {
    ...
    let field_data;
    const applyValidData = (physicalData) => {
        ...
        let physicalIndex = 0;
        return item.valid_data.map((valid) => {
            if (!valid) {
                return null;
            }
    ...
    switch (dataKey) {
        ...
        case 'int8_vector':
            ...
            for (let i = 0; i < int8Bytes.byteLength; i += int8Dim) {
                ...
                const signedSlice = new Int8Array(slice.buffer, slice.byteOffset, slice.byteLength);
                field_data.push(localTransformers[__1.DataType.Int8Vector](signedSlice));
            }
            break;
        case 'float16_vector':
            ...
            if (elementType == __1.DataType.Struct) {
                ...
                rowData[name].forEach((structElement, elementIndex) => {
                    ...
                    fieldNames.forEach(fieldName => {
                        ...
                        if (__1.VectorDataTypes.includes(structField.type)) {
                            dataArray.push(...(Array.isArray(fieldData) ? fieldData : [fieldData]));
                        }
                        else if (structField.type === __1.DataType.ArrayOfVector &&
                            (structField.elementType === __1.DataType.FloatVector ||
                                structField.elementType === __1.DataType.BinaryVector)) {
                            dataArray.push(...(Array.isArray(fieldData) ? fieldData : [fieldData]));
                        }
                        else if (structField.type === __1.DataType.ArrayOfVector &&
                            __1.VectorDataTypes.includes(structField.elementType)) {
