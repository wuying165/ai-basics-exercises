...
const normalizeOrderByFields = (orderByFields) => {
    ...
    return orderByFields
        ...
        if (!item || typeof item !== 'object') {
            throw new Error(`Invalid order_by_fields item format, expect string or object, got ${typeof item}`);
        }
        if (!item.field) {
            throw new Error("Invalid order_by_fields item: 'field' key is required and cannot be empty");
        }
        return `${item.field}:${(_a = item.order) !== null && _a !== void 0 ? _a : 'asc'}`;
    })
        .join(',');
};
exports.normalizeOrderByFields = normalizeOrderByFields;
/**
 *
