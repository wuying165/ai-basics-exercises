...
const omitOrderByFields = (data) => {
    const { order_by_fields, order_by } = data, rest = __rest(data, ["order_by_fields", "order_by"]);
    return rest;
};
const omitOrderByFieldsFromRequest = (data) => {
    const request = omitOrderByFields(data);
    return Object.assign(Object.assign({}, request), { params: request.params ? omitOrderByFields(request.params) : request.params });
};
/**
...
const buildSearchParams = (data, anns_field) => {
    ...
    const orderByFields = (0, __1.normalizeOrderByFields)(data.order_by_fields);
...
const buildSearchRequest = (params, collectionInfo, milvusProto) => {
    ...
    const userRequests = isHybridSearch
        ? searchHybridReq.data.map(d => (Object.assign(Object.assign({}, omitOrderByFieldsFromRequest(params)), omitOrderByFieldsFromRequest(d))))
        : [
    ...
    for (const userRequest of userRequests) {
        ...
        const metricType = (_b = (_a = annsField.index_params) === null || _a === void 0 ? void 0 : _a.find(param => param.key === 'metric_type')) === null || _b === void 0 ? void 0 : _b.value;
        const isPlainStructVectorMetric = metricType !== undefined && !String(metricType).startsWith('MAX_SIM');
        ...
        ].includes(annsField._placeholderType);
        const isEmbeddingListData = isEmbeddingListPlaceholder &&
            !isPlainStructVectorMetric &&
            Array.isArray(data) &&
            Array.isArray(data[0]);
        const placeholderType = annsField.is_function_output
            ? annsField._placeholderType
            : isEmbeddingListData
                ? annsField._placeholderType
                : annsField.dataType || __1.DataTypeMap[annsField.data_type];
        const searchData = ids && ids.length > 0
            ? []
            : (0, exports.formatSearchData)(data, Object.assign(Object.assign({}, annsField), { _placeholderType: placeholderType }));
        const request = {
        ...
        if (ids && ids.length > 0) {
            ...
            request.placeholder_group = (0, __1.buildPlaceholderGroupBytes)(milvusProto, searchData, annsField, placeholderType);
        }
        if (!isHybridSearch && params.cluster_id) {
            request.search_params = [
                ...request.search_params,
                { key: __1.CLUSTER_ID, value: params.cluster_id },
            ];
        }
     ...
     */
    const round_decimal = (_f = (_d = (_c = searchReq.search_params) === null || _c === void 0 ? void 0 : _c.round_decimal) !== null && _d !== void 0 ? _d : (_e = searchSimpleReq.params) === null || _e === void 0 ? void 0 : _e.round_decimal) !== null && _f !== void 0 ? _f : -1;
    // get rerank
    ...
    return Object.assign(Object.assign({ isHybridSearch: isHybridSearch, request: isHybridSearch
            ? Object.assign(Object.assign(Object.assign({ collection_name: params.collection_name, partition_names: params.partition_names, requests: requests, output_fields: (_g = requests[0]) === null || _g === void 0 ? void 0 : _g.output_fields, consistency_level: (_h = requests[0]) === null || _h === void 0 ? void 0 : _h.consistency_level }, (0, exports.createFunctionScore)(rerank)), {
                rank_params: [
                    ...(isRerankerObj
                    ...
                    {
                        key: 'limit',
                        value: (_k = (_j = searchSimpleReq.limit) !== null && _j !== void 0 ? _j : searchSimpleReq.topk) !== null && _k !== void 0 ? _k : __1.DEFAULT_TOPK,
                    },
                    {
                        key: 'offset',
                        value: (_l = searchSimpleReq.offset) !== null && _l !== void 0 ? _l : 0,
                    },
                    ...clusterIdParam,
                ],
...
const formatSearchResult = (searchRes, options) => {
    ...
    const results = [];
    const { topks, scores, fields_data, highlight_results, element_indices, group_by_field_values, } = searchRes.results;
    const elementOffsets = element_indices === null || element_indices === void 0 ? void 0 : element_indices.data;
    // build fields data map
    const fieldsDataMap = (0, __1.buildFieldDataMap)(fields_data, options.transformers);
    const groupByFieldValuesMap = (group_by_field_values === null || group_by_field_values === void 0 ? void 0 : group_by_field_values.length)
        ? (0, __1.buildFieldDataMap)(group_by_field_values, options.transformers)
        : undefined;
...
const formatSearchData = (searchData, field) => {
    ...
    switch (_placeholderType) {
        case __1.PlaceholderType.EmbListSparseFloatVector:
            throw new Error('Sparse embedding list search is not supported');
        case __1.PlaceholderType.EmbListFloatVector:
        case __1.PlaceholderType.EmbListBinaryVector:
        case __1.PlaceholderType.EmbListFloat16Vector:
        case __1.PlaceholderType.EmbListBFloat16Vector:
        case __1.PlaceholderType.EmbListInt8Vector: {
            const isTypedArray = (value) => value instanceof Uint8Array || value instanceof Int8Array;
            const flattenEmbeddingList = (embeddingList) => {
                if (Array.isArray(embeddingList) &&
                    embeddingList.length > 0 &&
                    embeddingList.every(isTypedArray)) {
                    const totalLength = embeddingList.reduce((total, vector) => total + vector.length, 0);
            ...
            const isMultiEmbeddingList = Array.isArray(searchData) &&
                ...
                (Array.isArray(searchData[0][0]) ||
                    isTypedArray(searchData[0][0]));
            if (isMultiEmbeddingList) {
                return searchData.map(v => flattenEmbeddingList(v));
            }
            else {
                return [flattenEmbeddingList(searchData)];
            }
        }
