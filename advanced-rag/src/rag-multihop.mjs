...
const RouteSchema = z.object({ strategy: z.enum(["simple", "complex"]), reason: z.string() })
...
async function retrieveRelevantContent(question, k) {
 try {
    const result = await vectorStore.similaritySearchWithScore(question, k)
    ...
    console.error("检索内容时出错:", error.messages)
    return []
  } 
}

// 按 id 合并；同 id 保留更高 score
function mergeUnique (exitingDocs, newDocs) {
  const map = new Map()

  for (const d of [...exitingDocs, ...newDocs]) {
...
const retrieveNode = async (state) => {
  ...
  const documents = await retrieveRelevantContent(q, state.k)
  const merged = mergeUnique(state.documents ?? [], documents);

  if (documents.length === 0) {
    console.log("本轮未命中文档");
  } else {
    console.log(`本轮命中 ${documents.length} 条，累计去重后 ${merged.length} 条`);
