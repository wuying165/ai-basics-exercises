...
async function main() {
  ...
  const searchResult = await client.search('用户的饮食限制是什么？中文回答。', { filters: { user_id: USER_ID }, topK: 5 })
  log('搜索记忆', searchResult)

  const allMemories = await client.getAll({ filters: { user_id: USER_ID }, pageSize: 10 })
  ...
  if (firstMemory?.id) {
    ...
    const updated = await client.update(firstMemory.id, { text: '更新后的内容' })
    log('更新记忆', updated)
