...
async function searchUserMemory(client) {
  const searchResult = await client.search('用户住在哪里，有什么爱好', { filters: { user_id: USER_ID }, topK: 5 })
  log('用户记忆 — search', searchResult.results?.map((m) => m.memory) ?? [])

  const listResult = await client.getAll({ filters: { user_id: USER_ID }, pageSize: 10 })
  log('用户记忆 — list', listResult.results?.map((m) => m.memory) ?? [])
}
...
async function searchSessionMemory(client) {
  const searchResult = await client.search('会话记忆 — search', { filters: { user_id: USER_ID, run_id: RUN_ID }, topK: 5 })
  log('会话记忆 — search', searchResult.results?.map((m) => m.memory) ?? [])

  const listResult = await client.getAll({ filters: { user_id: USER_ID, run_id: RUN_ID }, pageSize: 10 })
  log('会话记忆 — list', listResult.results?.map((m) => m.memory) ?? [])
}
...
async function searchAgentMemory(client) {
  const searchResult = await client.search('这个 Agent 的角色和回答方式', { filters: { agent_id: AGENT_ID }, topK: 5 })
  log('Agent 记忆 — search', searchResult.results?.map((m) => m.memory) ?? [])

  const listResult = await client.getAll({ filters: { agent_id: AGENT_ID }, pageSize: 10 })
  log('Agent 记忆 — list', listResult.results?.map((m) => m.memory) ?? [])
}
